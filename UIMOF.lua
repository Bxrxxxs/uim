...
function Kavo:DraggingEnabled(frame, parent)
    ...
    local dragInput, startPos, startParentPos

    local function StartDrag(inputPos)
        dragging = true
        startPos = inputPos
        startParentPos = parent.Position
    end
    ...
    input.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            ...
            parent.Position = UDim2.new(startParentPos.X.Scale, startParentPos.X.Offset + delta.X, startParentPos.Y.Scale, startParentPos.Y.Offset + delta.Y)
        end
    end)
